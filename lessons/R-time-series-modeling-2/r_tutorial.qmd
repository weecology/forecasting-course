...
> $y_t = c + \theta_1 e_{t-1} + \epsilon_t$

* So if $\theta_1$ > 0 then if the observation has greater than the mean at the previous time step it is like to be greater than the mean at the current time step
...
> $y_t = c + b_1 y_{t-1} + \theta_1 \epsilon_{t-1} + \epsilon_t$
...
* So the SAR1 part of the model is
...
> $y_t = c + \theta_1 e_{t-1} + \theta_2 e_{t-2} + \theta_3 e_{t-3} + b_{12} y_{t-12} + \epsilon_t$
