...
* It takes the form
  ...
  > $y_t = c + \epsilon_t$, where $\epsilon_t \sim N(0, \sigma)$
...
> *leave room to add $y_{t-2}$*

> $y_t = c + b_1 y_{t-1} + \epsilon_t$, where $\epsilon_t \sim N(0, \sigma)$
...
> $y_t = c + b_1 y_{t-1} + b_2 y_{t-2} + \epsilon_t$, where $\epsilon_t \sim N(0, \sigma)$

> Instructors note: Actually $y_t = (1 - b_1 - b_2) c + b_1 y_{t-1} + b_2 y_{t-2} + \epsilon_t$ due to non-zero mean
